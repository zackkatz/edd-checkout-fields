...
if ( ! class_exists( 'SF_Settings_API' ) ) {
	...
	class SF_Settings_API
	{
		...
		public function add_settings_link( $links, $file )
		{
			...
			if ( $file == $this_plugin ) {
				$settings_link = '<a href="'.admin_url('edit.php?post_type=fes-forms').'">' . __( 'Forms', 'edd_fes' ) . '</a>';
				array_unshift( $links, $settings_link );
				$settings_link = '<a href="'.admin_url('admin.php?page=edd_fes').'">' . __( 'Settings', 'edd_fes' ) . '</a>';
				array_unshift( $links, $settings_link );
				$settings_link = '<a href="'.admin_url('admin.php?page=fes-about').'">' . __( 'About', 'edd_fes' ) . '</a>';
				array_unshift( $links, $settings_link );
			}
			return $links;
		}
		...
		private function actions()
		{
			...
			add_action( 'admin_menu', array( &$this, 'create_menu' ),12 );
		...
		public function load_options( $option_file )
		{
			...
			if ( file_exists( $option_file ) ) {
				...
				wp_die( __( 'Could not load settings at: ', 'edd_fes' ) . '<br/><code>' . $option_file . '</code>', __( 'Error - WP Settings Framework', 'edd_fes' ) );
			}
		}
		...
		public function validate_options( $input )
		{
			...
			add_settings_error( $this->id, 'save_options', __( 'Settings saved.', 'edd_fes' ), 'updated' );
...
?>
			<div class="wrap">
				<?php screen_icon('options-general'); ?><h2><?php echo $this->title; ?></h2>
...
?>
				...
				<p class="submit">
					...
					<input type="submit" name="update" class="button-primary" value="<?php echo sprintf( __( 'Save %s Changes', 'edd_fes' ), $this->tab_headers[$tabname] ); ?>" />
				</p>
			</form> <?php
