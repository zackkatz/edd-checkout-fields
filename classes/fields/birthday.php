<?php
class CFM_Birthday_Field extends CFM_Field {
	...
	public function set_title() {
		$title = _x( 'Birthday', 'CFM Field title translation', 'edd_cfm' );
		$title = apply_filters( 'cfm_' . $this->name() . '_field_title', $title );
		$this->supports['title'] = $title;		
	}

	/** Returns the Birthday to render a field in admin */
	public function render_field_admin( $user_id = -2, $profile = -2 ) {
		...
		ob_start(); ?>
		<input name="<?php echo esc_attr( $this->name() ); ?>" id="<?php echo $this->name(); ?>" type="text" class="datepicker text edd-input" data-required="false" data-type="text" value="<?php echo esc_attr( $value ) ?>" />
		<script type="text/javascript">
			jQuery(function($) {
			<?php if ( $this->characteristics['time'] == 'yes' ) { ?>
				$("#<?php echo $this->name(); ?>").datetimepicker({ birthdayFormat: '<?php echo $this->characteristics['format']; ?>' });
			<?php } else { ?>
				$("#<?php echo $this->name(); ?>").datepicker({ birthdayFormat: '<?php echo $this->characteristics['format']; ?>' });
			<?php } ?>
			});
		</script>
		<?php
	...
	/** Returns the Birthday to render a field in frontend */
	...
	/** Returns the Birthday to render a field for the formbuilder */
	public function render_formbuilder_field( $index = -2, $insert = false ) {
		...
		$help         = esc_attr( __( 'The birthday format', 'edd_cfm' ) ); ?>
		<li class="custom-field custom_image">
			...
			<?php CFM_Formbuilder_Templates::field_div( $index, $this->name(), $this->characteristics, $insert ); ?>
				...
				<div class="cfm-form-rows">
					<label><?php _e( 'Birthday Format', 'edd_cfm' ); ?></label>
